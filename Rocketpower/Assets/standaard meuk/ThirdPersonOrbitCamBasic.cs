...
public class ThirdPersonOrbitCamBasic : MonoBehaviour 
{
	...
	void FixedUpdate()
	{
		...
		// Joystick:
		angleH += Mathf.Clamp(vc.HorizontalLook, -1, 1) * 60 * horizontalAimingSpeed * Time.deltaTime;
		angleV += Mathf.Clamp(-vc.VerticalLook, -1, 1) * 60 * verticalAimingSpeed * Time.deltaTime;
	...
	{
        float playerFocusHeight = player.GetComponent<CharacterController>().height;
